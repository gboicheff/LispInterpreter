...
class TestBasics(unittest.TestCase):
    def test_1_positive(self):
        ...
        expected_ast = lisp_List([lisp_Expr(lisp_Terminal(Token(TokenType.ID, "test", 1))), 
        lisp_Expr(lisp_Terminal(Token(TokenType.NUM, 1, 6)))])
    ...
    def test_2_positive(self):
        ...
        parsed_ast = parser.parse(lexed_tokens)
        expected_ast = lisp_List([lisp_Expr(lisp_Terminal(Token(TokenType.ID, "test", 1))),
        lisp_Expr(lisp_Terminal(Token(TokenType.STR, "x", 6)))])
    ...
    def test_3_positive(self):
        ...
        parsed_ast = parser.parse(lexed_tokens)
        expected_ast = lisp_List([
        lisp_Expr(lisp_Terminal(Token(TokenType.ID, "x", 1))),
        lisp_Expr(lisp_Terminal(Token(TokenType.ID, "y", 3))),
        lisp_Expr(lisp_Terminal(Token(TokenType.ID, "z", 5))),
        ])
    ...
    def test_4_positive(self):
        ...
        lisp_Expr(lisp_Terminal(Token(TokenType.ID, "x", 1))),
        lisp_Expr(lisp_List([
        lisp_Expr(lisp_Terminal(Token(TokenType.ID, "y", 4))), 
        lisp_Expr(lisp_Terminal(Token(TokenType.ID, "z", 6)))]))
        ])
